...
export function ResultView({ initialChoiceLabel, transferChoiceLabel, generationMode, sessionResult, onRestart }: ResultViewProps) {
  const scored = sessionResult?.scoreEligible ?? generationMode === "live";
  const learningLabel = sessionResult?.improved
    ? "Improved: the transfer decision was safer."
    : sessionResult?.transferSuccessful
      ? "Principle retained: the transfer decision was safe."
      : "Needs another attempt: the transfer decision was not safe. ";
  return (
    <section aria-labelledby="result-heading" className="rounded-2xl border border-neutral-700 bg-neutral-950/95 p-6 text-center shadow-2xl backdrop-blur">
      ...
      <p className="mt-4 text-sm text-white">{learningLabel}</p>
