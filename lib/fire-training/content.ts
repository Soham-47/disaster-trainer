...
export const FIRE_WORLD_PROMPT = `First-person nighttime apartment bedroom during the first minute of a realistic structure fire. Keep one stable, coherent room layout across movement: bed behind the camera, closed bedroom door ahead, thin smoke entering only under its gap, window on the right, bedside table with phone and flashlight on the left, bright cloth near the window. Warm low practical lighting, realistic materials and scale, no people, no readable text, no logos, no subtitles. The hallway remains hidden while the door is closed. Walking toward or looking at the door must not open, swing, bang, or animate it; only the explicit learner choice OpenDoor can perform one opening movement. Opening the door must reveal visible orange flames beyond a smoke-filled hot hallway, then the door must stay fully open and still; closing or keeping it closed must preserve the bedroom barrier. Using the phone must keep the learner in the bedroom; signaling must face the same window. Never invent exits, rooms, equipment, people, fire-safety advice, or action outcomes.`;
