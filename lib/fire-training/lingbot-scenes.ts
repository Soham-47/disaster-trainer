...
function scene(id: string, seed: number, branchPrompt: string): LingBotSceneContract {
  return { id, seed, invariantPrompt, branchPrompt, requiredVisualFacts: ["same bedroom geometry", "same learner viewpoint"], forbiddenVisualFacts, cameraPose: [], attentionWindow: "small", maximumFirstFrameMs: 60_000 };
}
...
export const FIRE_UNSAFE_SCENE = scene("fire-door-open", 77117, "The previously closed warm bedroom door is now visibly open inward. Dense dark smoke and intense orange hallway light enter through the opening. Visibility worsens quickly. The learner remains inside the bedroom and does not cross the doorway. Hold this consequence consistently.");
export const FIRE_SAFE_SCENE = scene("fire-door-closed", 77117, "The bedroom door visibly remains fully closed in the same position. Smoke stays concentrated beneath the door gap instead of filling the room. The learner remains inside near the window while communicating and signaling for help. Hold this consequence consistently.");
